...
-spec headers(Credentials :: credentials(), Service :: string(),
 ...
 % e.g., "bucketname.s3.amazonaws.com"
                                                         ...
                                                         % e.g., "20170101T000000Z"
 % e.g., "DynamoDB_20120810.CreateTable"
...
isonow() -> isonow(calendar:universal_time()).
...
canonical_query(QueryParams) when is_list(QueryParams) ->
    join($&,
         [[quote(K), $=, quote(V)]
          || {K, V} <- lists:keysort(1, QueryParams), V /= undefined]);
...
credentials_from_plist(Plist) ->
    #credentials{expiration = erliam_util:getkey(expiration, Plist),
                 ...
                 secret_access_key =
                     erliam_util:getkey(secret_access_key, Plist)}.
...
hexlify(Bin) -> [int_to_hex(X) || X <- binary_to_list(Bin)].
...
flattened(KVs) -> [{K, lists:flatten(V)} || {K, V} <- KVs].

flattened_headers(Args) -> flattened(apply(?MODULE, headers, Args)).

basic_headers_test() ->
    Actual = flattened_headers([#credentials{secret_access_key =
                                                 "secretkey",
    ...
    Expected = flattened([{"authorization",
                           ["AWS4-HMAC-SHA256 Credential=accesskey/20140629/us-east"
                            ...
                            ",SignedHeaders=host;x-amz-date;x-amz-security-token;x-"
                            "amz-target",
                            ",Signature=847fee48568298911772356fe332443bf2679c48fd4"
                            "2695a84aaa0d0e7f28c66"]},
                          {"x-amz-content-sha256",
                           "3fc9b689459d738f8c88a3a48aa9e33542016b7a4052e001aaa536"
                           "fca74813cb"},
                                ...
                                #{aws_date => "20150830T123600Z",
                                  ...
                                  query_params =>
                                      #{"Param2" => "value2",
                                        "Param1" => "value1"}}]),
    Expected = flattened([{"authorization",
                           ["AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE/20150830/us-ea"
                            "st-1/service/aws4_request",
                            ",SignedHeaders=host;x-amz-date",
                            ",Signature=b97d918cfa904a5beff61c982a1b6f458b799221646"
                            "efd99d3219ec94cdf2500"]},
                          {"x-amz-content-sha256",
...
aws4_example2_test() ->
    ...
    Expected = flattened([{"authorization",
                           ["AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE/20150830/us-ea"
                            "st-1/service/aws4_request",
                            ",SignedHeaders=host;x-amz-date",
                            ",Signature=28038455d6de14eafc1f9222cf5aa6f1a96197d7deb"
                            "8263271d420d138af7f11"]},
                          {"x-amz-content-sha256",
                           "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca49599"
                           "1b7852b855"},
                          {"host", "example.amazonaws.com"},
...
aws4_example3_test() ->
    ...
    Expected = flattened([{"authorization",
                           ["AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE/20150830/us-ea"
                            "st-1/service/aws4_request",
                            ",SignedHeaders=host;x-amz-date",
                            ",Signature=07ef7494c76fa4850883e2b006601f940f8a34d404d"
                            "0cfa977f52a65bbf5f24f"]},
                          {"x-amz-content-sha256",
                           "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca49599"
                           "1b7852b855"},
                          {"host", "example.amazonaws.com"},
