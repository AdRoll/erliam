...
headers(Credentials, #{service := Service,
                       region := Region} = Parameters, RequestPayload) ->
...
headers(#credentials{secret_access_key = SecretAccessKey,
        ...
        Service, Region, Host, AwsDate, TargetAPI,
    ...
    SigningKey = lists:foldl(fun (E, A) ->
...
canonical_query(QueryParams) when is_list(QueryParams) ->
    join($&, [[quote(K), $=, quote(V)]
              || {K, V} <- lists:keysort(1, QueryParams),
                 V /= undefined]);
...
headers_(Credentials, #{service := Service,
                        region := Region,
                        host := Host,
...
basic_headers_test() ->
    Actual = flattened_headers([#credentials{secret_access_key = "secretkey",
                                             access_key_id = "accesskey",
                                             security_token = "securitytoken"},
                                "kinesis", "us-east-1", "kinesis.us-east-1.amazonaws.com",
