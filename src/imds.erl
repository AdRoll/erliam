...
-export([role_name/0, zone/0, instance_id/0, public_hostname/0,
         get_session_token/0, imds_response/3, imds_response/4]).
...
role_name() -> imds_text("iam/security-credentials/").

-spec zone() -> {error, term()} | {ok, string()}.

zone() -> imds_text("placement/availability-zone").

-spec instance_id() -> {error, term()} | {ok, string()}.

instance_id() -> imds_text("instance-id").

-spec public_hostname() -> {error, term()} | {ok, string()}.

public_hostname() -> imds_text("public-hostname").
...
get_session_token() ->
    case role_name() of
      {ok, RoleName} ->
          Result = imds_tokens(["iam/security-credentials/",
                                http_uri:encode(RoleName)]),
...
-spec imds_response(string(), [string()], pos_integer()) -> {ok,
                                                             term()} |
                                                            {error, term()}.

imds_response(Url, MimeTypes, Timeout) ->
    RequestHeaders = [{"Accept", string:join(MimeTypes, ", ")}],
    case httpc:request(get, {Url, RequestHeaders}, [{timeout, Timeout}],
                       [{body_format, binary}], erliam:httpc_profile())
        of
...
imds_response(Url, MimeTypes, Timeout, Retries) ->
    call_with_retry(?MODULE, imds_response, [Url, MimeTypes, Timeout],
                    "Could not obtain IMDS response: ~p~n", Retries).
...
-spec imds_transform_response(string(), [string()],
                              function()) -> {error, term()} | term().
...
-spec imds_text_response(string()) -> {ok, string()} |
                                      {error, term()}.
...
-spec imds_token_response(string()) -> [{atom(), string()}] |
                                       {error, term()}.
    ...
    case jiffy:decode(Body) of
      {Plist} ->
          case erliam_util:getkey(<<"Code">>, Plist) of
            <<"Success">> -> {success, Plist};
            _ -> {error, failed_token_response}
          end;
...
-spec call_with_retry(atom(), atom(), list(), string(),
...
call_with_retry(_, _, _, _, _) -> {error, retries_exceeded}.

imds_url(Suffix) ->
    lists:flatten(["http://", ?IMDS_HOST, "/", ?IMDS_VERSION,
                   "/meta-data/", Suffix]).

imds_text(Suffix) -> imds_text_response(imds_url(Suffix)).

imds_tokens(Suffix) -> imds_token_response(imds_url(Suffix)).
...
metadata_response_to_proplist_test() ->
    Body = <<"{\"Code\":\"Success\",\"LastUpdated\":\"2014-10-17T15:"
             "17:07-07:00\",\"Type\":\"AWS-HMAC\",\"AccessKeyId\":\""
             "XYZZY\",\"SecretAccessKey\":\"FLOOBLE\",\"Token\":\"BA"
             "ZZLE\",\"Expiration\":\"2014-10-18T09:00:30Z\"}">>,
    Result = metadata_response_to_token_proplist(Body),
    Expected = [{expiration, "2014-10-18T09:00:30Z"},
                {access_key_id, "XYZZY"}, {secret_access_key, "FLOOBLE"},
                {token, "BAZZLE"}],
